...
from custom_types import *     #导入py文件中的每个类 ，√（无修改路径）      看import里面有无路径要改的？？？？？？？？？？？？？？？？？？？？
...
class GroundTruthGenerator:
    ...
    def update_target(self):
        if (len(self.opt.switches) == 0 or self.opt.switches[-1] < self.level) and self.target_mesh is None:
            self.target_mesh = mesh_utils.load_mesh(f'{const.RAW_MESHES}/{self.opt.mesh_name}')               #涉及路径 basketball和deer命名反了吗？？？？？？？？？？target应该就是template命名 那不就是/dropbox_raw/littlecow.obj
            self.target_mesh, self.target_transform = mesh_utils.to_unit_cube(self.target_mesh)

        elif self.level == 0:
            self.target_mesh = mesh_utils.load_mesh(f'{const.RAW_MESHES}/{self.opt.mesh_name}_0')             #这是利用level0.obj？？
            self.target_mesh, self.target_transform = mesh_utils.to_unit_cube(self.target_mesh)
#意思为：一开始从{const.RAW_MESHES}/里面加载原始target模型，如果level=0那么。。。不会了？？？？？？？？？？？？？？？？？？
        ...
        self.target_mesh = mesh_utils.to(self.target_mesh, self.device)      #路径太糊涂了？？？？？？？？？？？？？？？？？？？？？？
    ...
    def save_scales(self):
        with open(f'{self.opt.cp_folder}/{self.opt.mesh_name}_scales.pkl', 'wb') as f:  #google云盘上的meshname对应是lizard？？？写反了吗？？？？？
